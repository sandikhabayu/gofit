...
export function useWorkoutData() {
  ...
  const addWorkout = async (workout: any) => {
    try {
      ...
      // Update today's workout if it's for today
  ...
  const deleteWorkout = async (workoutId: string) => {
    try {
      ...
      await workoutStorage.saveWorkouts(filteredWorkouts);
      
      // Update today's workout if it was deleted
      if (todayWorkout && todayWorkout.id === workoutId) {
  ...
  const addExercise = async (exercise: any) => {
    try {
      ...
      // SOLUSI: Panggil loadAllData untuk menyinkronkan state di seluruh aplikasi
      await loadAllData();
